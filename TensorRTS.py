...
LinearRTS, the first epoch of TensorRTS, is intended to be the simplest RTS game.
    ...
    def act(self, actions: Mapping[ActionName, Action], trigger_default_opponent_action : bool = True, is_player_two : bool = False) -> Observation:
        ...
        if action.label == "advance":
            if self.tensors[0][0] < self.mapsize -1:
                player_tensor[0] += 1
                player_tensor[2] += self.collect_dots(player_tensor[0])
                player_tensor[0] += 1
                player_tensor[2] += self.collect_dots(player_tensor[0])
            elif self.tensors[0][0] < self.mapsize:
                player_tensor[0] += 1
                player_tensor[2] += self.collect_dots(player_tensor[0])
    ...
    def rush_ai(self): # This is the rush AI.
    ...
    def boom_ai(self): # This is the boom AI.
        seed = random.randint(1, 5)
        if (seed == 1):
            return self.rush_ai()
        else:
            self.tensors[1][2] += max(1,int(0.2 * self.tensors[1][2]))
            
    def random_ai(self): # This is the random AI
        ...
        if seed == 1:
            ...
            self.tensors[1][0] -= 1
            self.tensors[1][2] += self.collect_dots(self.tensors[1][0])
        elif seed == 2 and self.tensors[1][0] < self.mapsize:
            self.tensors[1][0] += 1
            self.tensors[1][2] += self.collect_dots(self.tensors[1][0])
        elif seed == 3:
            self.tensors[1][2] += max(1,int(0.2 * self.tensors[1][2]))
